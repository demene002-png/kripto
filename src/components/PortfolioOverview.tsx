...
export default function PortfolioOverview(){
  ...
  const rows=useMemo(()=>state.portfolio.map(item=>{
    ...
    const cost=item.investedUsdt && item.investedUsdt>0 ? item.investedUsdt : item.amount*item.averageBuyPrice;
    const value=item.amount*price;
    const estimatedExitFee=value*FEE_RATE;
    const pnl=value-estimatedExitFee-cost;
    return {...item,currentPrice:price,currentValue:value,cost,estimatedExitFee,pnl,pnlPct:cost>0?pnl/cost*100:0};
  ...
  const estimatedExitFees=rows.reduce((s,x)=>s+x.estimatedExitFee,0);
  const unrealized=rows.reduce((s,x)=>s+x.pnl,0);
  const total=state.balance+current;
  const estimatedNetLiquidation=total-estimatedExitFees;
  ...
  const handleClose=async(symbol:string)=>{ if(closingSymbol)return; setClosingSymbol(symbol); try{await closePosition(symbol)}finally{setClosingSymbol(null)} };
  ...
  return <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-5">
    ...
    <div className="grid md:grid-cols-2 gap-8"><div className="space-y-4"><div className="p-4 bg-zinc-800/30 rounded-xl"><div className="text-sm text-zinc-500">Toplam Varlık (Sanal)</div><div className="text-3xl font-bold">{formatCurrency(total)}</div><div className="text-[11px] text-zinc-500 mt-1">Tahmini satış komisyonları sonrası: {formatCurrency(estimatedNetLiquidation)}</div></div><div className="grid grid-cols-2 lg:grid-cols-4 gap-3"><div className="p-3 bg-zinc-800/30 rounded-xl"><div className="text-xs text-zinc-500">USDT Bakiye</div><div className="text-base">{formatCurrency(state.balance)}</div></div><div className="p-3 bg-zinc-800/30 rounded-xl"><div className="text-xs text-zinc-500">Açık Maliyet</div><div className="text-base">{formatCurrency(invested)}</div></div><div className="p-3 bg-zinc-800/30 rounded-xl"><div className="text-xs text-zinc-500">Gerçekleşmiş Net K/Z</div><div className={`text-base ${state.realizedPnl>=0?'text-emerald-400':'text-rose-400'}`}>{state.realizedPnl>=0?'+':''}{formatPnl(state.realizedPnl)}</div></div><div className="p-3 bg-zinc-800/30 rounded-xl"><div className="text-xs text-zinc-500">Açık Tahmini Net K/Z</div><div className={`text-base ${unrealized>=0?'text-emerald-400':'text-rose-400'}`}>{unrealized>=0?'+':''}{formatPnl(unrealized)}</div></div></div><div className="text-[11px] text-zinc-500 border border-zinc-800 rounded-lg px-3 py-2">Toplam simüle komisyon: <span className="text-zinc-300">{formatPnl(state.totalFees)}</span> · Komisyon oranı: alışta %0,10 + satışta %0,10. Açık K/Z, pozisyon şimdi kapatılsa ödenecek tahmini satış komisyonunu da içerir.</div></div>
