...
export default function TradeHistoryCard(){
  ...
  const recentSells=sells.slice(0,12);
  const recentWins=recentSells.filter(x=>x.realizedPnl>0);
  const recentLosses=recentSells.filter(x=>x.realizedPnl<0);
  const recentWinRate=recentSells.length?recentWins.length/recentSells.length*100:0;
  const grossProfit=recentWins.reduce((a,x)=>a+x.realizedPnl,0);
  const grossLoss=Math.abs(recentLosses.reduce((a,x)=>a+x.realizedPnl,0));
  const recentPf=grossLoss>0?grossProfit/grossLoss:(grossProfit>0?99:0);
  const entryFor=(r:TradeHistoryItem)=>{
    if(r.side==='BUY')return r;
    const t=new Date(r.createdAt).getTime();
    return rows.find(x=>x.side==='BUY'&&x.symbol===r.symbol&&new Date(x.createdAt).getTime()<=t);
  };
  ...
  return <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-5">
    ...
    <div className="grid grid-cols-2 md:grid-cols-7 gap-3 mb-4"><Metric label="Başlangıç" value={formatPnl(state.startingBalance)}/><Metric label="Gerçekleşmiş Net K/Z" value={`${state.realizedPnl>=0?'+':''}${formatPnl(state.realizedPnl)}`} tone={state.realizedPnl>=0?'good':'bad'}/><Metric label="Toplam Komisyon" value={formatPnl(state.totalFees)}/><Metric label="Kârlı Kapanış" value={String(wins)} tone="good"/><Metric label="Zararlı Kapanış" value={String(losses)} tone="bad"/><Metric label="Son 12 Kazanma" value={`%${recentWinRate.toFixed(0)}`} tone={recentWinRate>=50?'good':'bad'}/><Metric label="Son 12 Kâr Faktörü" value={recentPf>=99?'∞':recentPf.toFixed(2)} tone={recentPf>=1?'good':'bad'}/></div>
    ...
    <div className="overflow-x-auto max-h-[420px] overflow-y-auto"><table className="w-full text-xs"><thead className="sticky top-0 bg-zinc-950"><tr className="text-zinc-500"><th className="text-left py-2">Tarih</th><th className="text-left">Varlık</th><th className="text-left">İşlem</th><th className="text-right">Fiyat</th><th className="text-right">Miktar</th><th className="text-right">Komisyon</th><th className="text-right">Net K/Z</th><th className="text-right">Net %</th><th className="text-left pl-3">Giriş Kalitesi</th><th className="text-left pl-3">Neden</th></tr></thead><tbody>{rows.map(r=>{const e=entryFor(r);return <tr key={r.id} className="border-t border-zinc-800/60"><td className="py-2 text-zinc-400 whitespace-nowrap">{new Date(r.createdAt).toLocaleString('tr-TR')}</td><td className="font-medium">{shownSymbol(r.symbol)}</td><td className={r.side==='BUY'?'text-sky-400':'text-amber-300'}>{r.side==='BUY'?'AL':'SAT'}</td><td className="text-right">{formatAssetPrice(r.price)}</td><td className="text-right">{r.quantity.toLocaleString('tr-TR',{maximumFractionDigits:6})}</td><td className="text-right">{formatPnl(r.feeUsdt)}</td><td className={`text-right ${r.side==='SELL'?(r.realizedPnl>=0?'text-emerald-400':'text-rose-400'):'text-zinc-500'}`}>{r.side==='SELL'?`${r.realizedPnl>=0?'+':''}${formatPnl(r.realizedPnl)}`:'-'}</td><td className={`text-right ${r.side==='SELL'?(Number(r.netReturnPct||0)>=0?'text-emerald-400':'text-rose-400'):'text-zinc-500'}`}>{r.side==='SELL'?`${Number(r.netReturnPct||0)>=0?'+':''}${Number(r.netReturnPct||0).toFixed(3)}%`:'-'}</td><td className="pl-3 text-zinc-300 min-w-[170px]">{e&&e.opportunity!=null?<><div className="font-medium">Skor {Number(e.opportunity).toFixed(0)} • Risk {Number(e.risk||0).toFixed(0)} • Güven {Number(e.confidence||0).toFixed(0)}</div><div className="text-[10px] text-zinc-500 mt-0.5">{e.primaryStrategy||'-'} • {e.marketRegime||'-'}</div></>:<span className="text-zinc-600">-</span>}</td><td className="pl-3 text-zinc-300">{reasonLabel(r.reason)}</td></tr>})}{rows.length===0&&!loading&&<tr><td colSpan={10} className="text-center py-8 text-zinc-500">Henüz işlem kaydı yok.</td></tr>}</tbody></table></div>
  </div>
}
