...
const styles:Record<string,string>={
  ...
  HIGH_RISK:'bg-orange-500/10 text-orange-300 border-orange-500/20',
  VETO:'bg-rose-500/10 text-rose-300 border-rose-500/20'
};
...
export default function NewsIntelligenceCard(){
  ...
  return <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-5">
    <div className="flex items-start justify-between gap-4 mb-4">
      <div><div className="flex items-center gap-2 text-zinc-100 font-medium"><Newspaper size={18} className="text-violet-400"/> News & Event Intelligence</div><p className="text-xs text-zinc-500 mt-1">Haber tek başına AL üretmez; doğrulanmış kritik negatif olaylar VETO uygulayabilir.</p></div>
      <span className={`px-3 py-1.5 rounded-lg text-xs font-semibold border ${styles[n.level]||styles.INFO}`}>{n.level}</span>
    </div>
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm mb-4">
      <Metric icon={<Radio size={13}/>} label="Sentiment" value={`${n.score.sentiment>0?'+':''}${n.score.sentiment}`}/>
    ...
    {n.news.length>0?<div className="space-y-2">
      {n.news.slice(0,3).map(x=><div key={x.id} className="flex items-start justify-between gap-3 text-xs border-t border-zinc-800 pt-2"><div><div className="text-zinc-300">{x.title}</div><div className="text-zinc-600 mt-1">{x.source} • güven {x.reliability}/100 • {x.category}{x.confirmations>1?` • ${x.confirmations} teyit`:''}</div></div><span className={x.sentiment<0?'text-rose-400':'text-emerald-400'}>{x.sentiment>0?'+':''}{x.sentiment}</span></div>)}
    </div>:<div className="text-xs text-zinc-500">Canlı haber sağlayıcısı henüz yapılandırılmadı veya güncel kayıt yok. Sistem bu durumda haber skoru uydurmaz.</div>}
