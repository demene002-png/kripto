...
const labels: Record<string,string> = {
  STRONG_BULL:'Güçlü Yükseliş', BULL:'Yükseliş', SIDEWAYS:'Yatay', HIGH_VOLATILITY:'Yüksek Oynaklık', BEAR:'Düşüş', PANIC:'Panik'
};
...
export default function MarketRegimeCard(){
  ...
  return <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-5">
    <div className="flex items-center justify-between gap-4 mb-4">
      <div>
        <div className="flex items-center gap-2 text-zinc-100 font-medium"><Activity size={18} className="text-blue-400"/> Piyasa Rejim Motoru</div>
        <p className="text-xs text-zinc-500 mt-1">BTC/ETH eğilimi + ilk 50 coin piyasa genişliği + oynaklık</p>
      </div>
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
      ...
      <div className="bg-zinc-950/60 rounded-lg p-3 border border-zinc-800"><span className="text-zinc-500 text-xs flex items-center gap-1"><TrendingDown size={13}/> Ort. 24 saat</span><div className="text-zinc-100 font-semibold mt-1">{marketRegime.avgChange24h>0?'+':''}{marketRegime.avgChange24h}%</div></div>
      <div className="bg-zinc-950/60 rounded-lg p-3 border border-zinc-800"><span className="text-zinc-500 text-xs flex items-center gap-1"><ShieldAlert size={13}/> Oynaklık</span><div className="text-zinc-100 font-semibold mt-1">{marketRegime.volatility}</div></div>
    </div>
  </div>
}
